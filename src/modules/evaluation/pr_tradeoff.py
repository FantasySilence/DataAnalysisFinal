...
class PRTradeOffCurve(EvaluateObjectBase):
    ...
    def __init__(
        ...
        y_score: 预测值, 概率
        avg_method: 宏平均(多分类与二分类均适用), 微平均(只适用于多分类)
    ...
    def draw(self) -> None:
        ...
        if self.avg_method == "micro":
            if self.n_class > 2:
                self.__micro_multiclass__()
            else:
                raise ValueError("n_class must be greater than 2")

        # ------ 宏平均 ------ #
        elif self.avg_method == "macro":
            self.__macro_multiclass__()
    ...
    def __micro_multiclass__(self) -> None:
    ...
    def __macro_multiclass__(self) -> None:
